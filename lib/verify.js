...
module.exports = (m) => {
  ...
  return db().collection('sales').find({ license_key: m.licenseKey })
    ...
    .then((sales) => {
      ...
      // since 1.1.2017 gumroad is sending also purchaser_id, in the previous sales we need to pair just by email
      // in the newer sales we user both purchaser_id and email, the email is likely not needed but to be sure
